...
- AiAdvisor：只做轻量 server-side 建议，比如任务标题生成、workspace strategy、提交/PR 文案草稿。它不读写仓库，不执行命令，不操作 Git。
...
`POST /api/tasks` 的 `title` 字段是可选字段。Web UI 新建任务时只提交仓库、任务内容、优先级、标签和范围；后端创建任务后通过 AiAdvisor 生成标题并回写，再返回包含 `repository` 和 `latestRun` 的 `TaskWithLatestRun`。如果未配置 AI 或标题生成失败，创建请求不被阻塞，任务会保留由任务内容生成的本地回退标题。
